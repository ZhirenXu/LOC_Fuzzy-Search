...
def fuzzyNameSearch(*names):
    ...
    for data in dataList:
        if data == []:
            ...
            if userChoice == -1:
                nameData.insert(0, [names[0][i], containerCluster[userChoice - 1].title, "null", "null", "null"])
            else:
                nameData.insert(0, [names[0][i], containerCluster[userChoice - 1].title, containerCluster[userChoice - 1].LC_URI, containerCluster[userChoice - 1].concept, containerCluster[userChocie - 1].subdivision])
            containerCluster = []
        i = i + 1
    return nameData
...
def fuzzySubjectSearch(*subjects):
    ...
    for data in dataList:
        if data == []:
            ...
            #load list of data into a list
            if userChoice == -1:
                subjectData.insert(0, [subjects[0][i], containerCluster[userChoice - 1].title, "null", "null", "null"])
            else:
